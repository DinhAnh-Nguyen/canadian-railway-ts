...
export async function PUT(request: Request, { params }: { params: { id: Number | String } }) {
    ...
    const response = await sql`UPDATE tasks SET description = ${resquestData.description}, status = ${resquestData.status}, due_date = ${resquestData.due_date}, piroty = ${resquestData.priority} WHERE id = ${id};`;
    return new Response(JSON.stringify(response), { status: 200 });
}
...
export async function DELETE(request: Request, { params }: { params: { id: number } }) {
    ...
    const response = await sql`DELETE FROM tasks WHERE id = ${params.id};`;
